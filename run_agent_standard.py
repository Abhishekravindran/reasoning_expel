...
import pandas as pd
import signal
from rca.baseline.rag_agent import RAGAgent
from rca.baseline.rca_agent.controller import control_loop_rag
...
def main(args, uid, dataset):
    ...
    # Pass schema_summary to prompt modules for all datasets
    ...
    if args.mode == 'rag':
        rag_agent = RAGAgent(dataset, bp.schema)
    else:
    ...
    for idx, row in instruct_data.iterrows():
        ...
        for i in range(args.sample_num):
            ...
            try: 
                ...
                if args.mode == 'rag':
                    prediction, trajectory = control_loop_rag(instruction, rag_agent, logger, max_turn=args.controller_max_turn)
                    prompt = None
                else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mode", type=str, default='sql', choices=['sql', 'rag'])
