...
def control_loop_rag(query, rag_agent, logger, max_turn=3):
    """
    Advanced RAG feedback/self-correction loop.
    - query: the user question/instruction
    - rag_agent: an instance of RAGAgent
    - logger: logger for info/debug
    - max_turn: max number of retrieval/LLM steps
    ...
    for turn in range(max_turn):
        logger.info(f"RAG Step {turn+1} for query: {query}")
        answer, retrieved = rag_agent.run(query, logger, top_k=5+turn*5)  # Expand context each turn
        trajectory.append({
            'step': turn+1,
            'retrieved': [r['passage'] for r in retrieved],
            'answer': answer
        })
        # Heuristic: if answer is sufficient, break
        if not any(x in answer.lower() for x in ["not enough context", "need more info", "insufficient", "unknown", "cannot answer"]):
            best_answer = answer
            best_score = turn
            break
        # Otherwise, try again with more context
        best_answer = answer  # fallback to last answer if all are insufficient
        best_score = turn
    logger.info(f"Final RAG answer (after {best_score+1} steps): {best_answer}")
    return best_answer, trajectory
